...
def test_list_all_breeds(base_url):
    """Testing the API endpoint for listing all breeds."""
...
# Тоже не понимаю, как интегрировать xfail в параметры, если вообще так надо делать (у меня так тест не сработал):
# @pytest.mark.parametrize('param', pytest.param([str(i) for i in [0, 1000000000]],
#                                                marks=pytest.mark.xfail(reason="Jira-1234")))

@pytest.mark.xfail
@pytest.mark.parametrize('param', [str(i) for i in [0, 1000000000]])
def test_api_random_image_zero(base_url, random_param, param):
